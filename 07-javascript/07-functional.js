...
const mapNameFamily = (characters) => {
  ...
  const mappedCharacters = characters.map(
...
const reduceHouses = (characters) => {
  ...
  const houseCount = characters.reduce((total, character) => {
    total.hasOwnProperty(character.house) ? total[character.house]++ : (total[character.house] = 1);
    return total;
  }, {}); //why is this required to prevent extranios data?
  return houseCount;
};
